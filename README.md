...
Current registry baseline after the next release: `@promptframe/contracts@0.1.6`, `@promptframe/component-kit@0.1.7`, `@promptframe/cli@0.1.18`, `create-promptframe-component@0.1.11`. `@promptframe/contracts@0.1.6` exposes the public authoring standard release, upload target policy, freshness decision schema, and component reusability score schema. `@promptframe/component-kit@0.1.7` sources its public standard stamp and style helper contract from `@promptframe/contracts`, and exposes `createPreviewCaseMatrix()` for bounded local preview aspect / props stress cases; `@promptframe/cli@0.1.18` includes `check` / `upgrade` lifecycle diagnostics, blocks stale PromptFrame authoring package floors before network upload for both folders and source zip archives, checks the platform standard source hash before sending package bytes, reports offline degraded freshness for `dev` / `check` when no platform endpoint is available, returns `localReusability` diagnostics from `check` and directory `upload`, can forward explicit dev-header auth roles / permissions to guarded local platform endpoints, writes local preview reports from canonical and saved preview cases, and ignores package manager lockfiles during source safety scans and source package creation. `create-promptframe-component@0.1.11` scaffolds templates that use current public package ranges, include a Remotion Player local preview shell with schema-derived props controls, 16:9 / 9:16 / 1:1 aspect presets, bounded auto preview cases, saved local preview case export/reporting for `.promptframe/local-previews/`, and support explicit `--name` / `--force` for legacy wrapper compatibility.
